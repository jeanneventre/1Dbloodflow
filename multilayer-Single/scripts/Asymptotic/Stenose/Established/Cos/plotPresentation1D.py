...
def main(argv) :
    ...
    for dRstr in ["-0.4","+0.4"] :
        ...
        for pType in ["Tw","P"] :
            ...
            lLabel      = [
                            "Reference","Multiring","1D, " + r"$C_f$ = $8\pi\nu$"
                            ]
            ...
            lXScale     = [ 1.,Re_c*R_c]
            lYScale     = [ YScaleRef,YScale]
            lYOffset    = [ YOffsetRef,YOffset]
            liY         = [ iYRef,iY]
            lCol        = [ "red","blue" ]
            lStyle      = [ "-","--" ]
            lFile       = [ Steady,Sten ]
            ...
            title       = "Presentation_" + pType + "_dR_" + dRstr + "_2D_Nx_" + str(Nx) + "_Nr_" + str(Nr) + ".pdf"
            ...
            lXScale     = [ 1.,Re_c*R_c ]
            lYScale     = [ YScaleRef,YScale1D ]
            lYOffset    = [ YOffsetRef,YOffset1D]
            liY         = [ iYRef,iY1D ]
            lCol        = [ "red","SeaGreen" ]
            lStyle      = [ "-","-." ]
            lFile       = [ Steady,Sten1_1D ]

            lLabel      = [
                            "Reference","1D " + r"$C_f$=$8\pi\nu$"
                            ]
            lText       = [r"$R_{e,R}$="+str(Re_c),r"$N_x$=" + str(Nx)]

            title       = "Presentation_" + pType + "_dR_" + dRstr + "_1D_Cf_8_Nx_" + str(Nx) + ".pdf"
            ...
            lYOffset    = [ YOffsetRef,YOffset,YOffset1D]
            liY         = [ iYRef,iY,iY1D ]
            lCol        = [ "red","blue","SeaGreen" ]
            lStyle      = [ "-","--","-." ]
            lFile       = [ Steady,Sten,Sten1_1D ]

            lLabel      = [
                            "Reference","2D","1D " + r"$C_f$=$8\pi\nu$"
                            ]
            lText       = [r"$R_{e,R}$="+str(Re_c),r"$N_x$=" + str(Nx) + r" $\rvert$ $N_r$=" + str(Nr)]

            title       = "Presentation_" + pType + "_dR_" + dRstr + "_2D_1D_Cf_8_Nx_" + str(Nx) + ".pdf"
