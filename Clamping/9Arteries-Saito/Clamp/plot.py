...
def main(argv) :
    ...
    if (float(nuv) == 5e4) and (float(Q) == 700) and (float(R1) == 200 )  and (float(Rt2) == 500) and (float(C) == 1e-6) :
        os.remove(fileName)
        # fh = open(fileName, 'w')
        # fh.write(" nuv, \t E, \t Rt, \t a,\t b, \t (a-b) \t \n")

    fh = open(fileName, 'a')
    fh.write("%d, \t %.3f, \t %.20f, \t %.20f, \t %.20f, \t %.20f, \t %.20f,  \t %.20f"%(float(Q),float(R1), float(Rt2), float(C), R2, Linf, L1,L2) + "\n")
    # -----------------------------------------------------
    # ---- PLOT RESULTS
    # # TEST PLOT
    ax = plt.gca()
