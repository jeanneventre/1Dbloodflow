...
# LINUX
export folderGeneral=/Documents/Boulot/Thèse/code/bloodflow/bloodflow/Examples/Clamping/9Arteries-Saito/Clamp/Phase
#MAC 
  ...
  for conj in "jS" ; do
    for nuv in "5e4" ; do 
      for Q in 200 250 300 350 400 450 500 550 600 650 700; do 
                      ...
                      for solver in "KIN_HAT"; do
                        for HR in HRQ; do
                          ...
                          export folderHR=${folderGeneral}/${NN}/${conj}/nuv=${nuv}/Q=${Q}/P1=${P1}/R2=${R2}/C=${C}/Nx=${Nx}/xOrder=${xOrder}/dt=${dt}/tOrder=${tOrder}/${solver}/${HR}
                           ...
                           # python3 writeParameter.py -a ${HOME}${folderHR}/ -b Sane -c ${conj} -d ${solver} -e ${HR} -f ${Nx} -g ${xOrder} -i ${dt} -j ${tOrder} -l ${NN} -n ${nuv} -t ${Q} --P1 ${P1} --P2 ${R2} -x ${C}  #-m ${phi} -n ${Cv} 
                           # bloodflow -i ${HOME}${folderHR}/parameters_Sane/ -o ${HOME}${folderHR}/data/ -s Sane
                           # python2.7 write.py -p ${HOME}${folderHR}/
                            # python3 plot.py -a ${HOME}${folderHR}/ -b Sane -c ${conj} -d ${solver} -e ${HR} -f ${Nx} -g ${xOrder} -i ${dt} -j ${tOrder} -l ${NN} -n ${nuv} -t ${Q} --P1 ${P1}
        ...
        # python3 phase2.py -a ${HOME}${folderHR}/ -b Sane -c ${conj} -d ${solver} -e ${HR} -f ${Nx} -g ${xOrder} -i ${dt} -j ${tOrder} -l ${NN} -n ${nuv} -t ${Q} --P1 ${P1} --P2 ${R2} -x ${C}
      done
    done
  done 
done
