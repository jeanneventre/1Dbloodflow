...
def main(argv) :
    ...
    Q = np.array([300, 350, 400, 450, 500, 550, 600, 650, 700])    
    Tej = np.array([0.25, 0.275, 0.3, 0.325, 0.35, 0.4, 0.45, 0.5])
    ...
    Cs.cmap.set_over('blakc')

    CS4 = plt.contour(X,Y, D, [-2,0,0.2],
                  colors=('k',),
                  linewidths=(3,),
                  origin='upper')
    ...
    plt.plot(0.45,300, 'b*',ms=10, label = "Pre Clamp optimum")
    plt.plot(0.36,600, 'r*',ms=10, label = "Post Clamp optimum")
    plt.legend()
    plt.colorbar(Cs)
    # plt.savefig('preclamp_phase_diagram.eps')
    plt.show()
