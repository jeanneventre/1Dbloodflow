...
def main(argv) :
    ...
    for State in ["Sane","Iliac","Aorta"] :

        PATH    = PATH1D + State + "/" + NNstr + "/" + Conjstr + "/Nx=" + Nxstr + "/xOrder=" + xOrderstr + "/dt=" + dtstr + "/tOrder=" + tOrderstr + "/" + Solverstr + "/" + HRstr + "/"
        ...
        for pType in ["P"] :
            ...
            for Time in [ ts + 0.*T_c, ts + 0.25*T_c, ts + 0.5*T_c, ts + 0.75*T_c] :

                lText       = [r"$t$=" + str(Time) + r"$T_h$",r"$Wang$ $55$-$Artery$ $Network$"]
                lTextPos    = [[0.01,.93],[0.5,.01]]
                lTextAlign  = ["left","center"]
                lTextColor  = ["black","black"]

                title   = "Network-Wang-" + State + "-" + pType + "-t-0p" + str(int((Time-ts)*100.)) + "T.pdf"
